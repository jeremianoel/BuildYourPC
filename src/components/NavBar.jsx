...
const NavBar = () => {
    ...
    return (
        <>
        <nav className="fixed top-0 left-0 w-full flex justify-between bg-[rgb(255,255,255,0.99)] z-50">
            <div className='flex justify-center'>
            <Link to="/"><img src={Logo} className="brightness-95 w-80 h-17 my-4 duration-300 cursor-pointer ml-30 hover:scale-105"/>
            </Link>
            </div>
            <ul className="flex m-auto mr-10 space-x-10 text-xl text-end">
                <li className="px-5 list-none"><Link className="px-5 py-2 no-underline duration-300
                 text-cyan-600 rounded-3xl hover:bg-cyan-600 hover:text-white" to={`/login`}>Login</Link></li>
                <li className="px-5 list-none"><Link className="px-5 py-2 no-underline duration-300
                 text-cyan-600 rounded-3xl hover:bg-cyan-600 hover:text-white" to={`/register`}>Register</Link></li>
            </ul>
        </nav>
        </>
    )
