...
const RegisterPage = () => {
    ...
    const handleRegister = async (e) => {
        ...
        if (password !== confirmPassword) {
            setTimeout(() => {
                setMessage(('Password needs to match!'));
            }, 500)
            return;
        } 
    ...
    return (
        <>
    <div className="container min-h-screen flex justify-center gap-20 items-center bg-[url('./assets/register.png')] bg-cover bg-center bg-fixed">
       <div className='flex justify-center'>
                <div className='flex flex-col justify-center bg-white px-3 py-8 items-center rounded-lg mt-10'>
                    <div className='flex'>
                        <p className='flex justify-center text-lg md:w-95'>Register your account</p>
                    </div>
                    <form onSubmit={handleRegister} action="" className='flex flex-col gap-7 text-lg px-5 mt-5'>
                    ...
                    {message && (
                        <div className='mt-5'>
                            <p className={`text-center text-xl font-semibold ${message === "Successfuly registered ✓" ? 'text-cyan-400' : 'text-red-600'}`}>{message}</p>
                        </div>
                    )}
        ...
        Got an account?
        <Link to='/login' className='text-cyan-400 ml-10 mr-2 hover:text-cyan-300 hover:cursor-pointer duration-100'>
            Login
        </Link>
        </p>
        </div>
